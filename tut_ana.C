...
void tut_ana(int nevts = 0, TString prefix = "D0_kpi")
{
    ...
    TH1F *hD0p_all = new TH1F("hD0p_all", "D0 momentum (all)", 200, -15, 15);
    TH1F *hD0cos_all = new TH1F("hD0cos_all", "D0 costheta (all)", 200, -1.5, 1.5);
    TH1F *hD0phi_all = new TH1F("hD0cos_all", "D0 costheta (all)", 200, -3.5, 3.5);
    ...
    while (theAnalysis->GetEvent() && i++ < nevts)
    {
        ...
        // All final particles are being fed into the lists
        ...
        // Make Histograms of P, CosTheta, Phi of the particles read from simulation
        // This happens without any previous calculations and sets a ground truth
        // If an MC-Truth is available, a Histogram will be filled, showing the K+ Particle ID
        // If the MC-Truth contains Mothers to K+ their IDs will also be put into a diagram
        // This process runs for all final particles
        for (j = 0; j < kplus.GetLength(); ++j)
        {
            ...
            hmkp_           ->Fill(kplus[j]->PdgCode()); // Fills Histogram with only one value
            hmkp            ->Fill(kplus[j]->GetMcTruth()->TheMother()->PdgCode());            
        }
        // kminus loop
        for (j = 0; j < kminus.GetLength(); ++j)
        {
            ...
            hmkm            ->Fill(kminus[j]->GetMcTruth()->TheMother()->PdgCode());
        }
        // piplus loop
        for (j = 0; j < piplus.GetLength(); ++j)
        {
            ...
            hmpp            ->Fill(piplus[j]->GetMcTruth()->TheMother()->PdgCode());
        }
        ...
        for (j = 0; j < piminus.GetLength(); ++j)
        {
            ...
            hmpm            ->Fill(piminus[j]->GetMcTruth()->TheMother()->PdgCode());
        }
        ...
        for (j = 0; j < D0.GetLength(); ++j)
        {
            ...
            hD0p_all->Fill(D0[j]->P());
            hD0cos_all->Fill(D0[j]->GetPosition().CosTheta());
            hD0phi_all->Fill(D0[j]->GetPosition().Phi());
        ...
        */
        ...
        for (j = 0; j < kplus_lo.GetLength(); ++j)
        {
            ...
            if (kplusMcTruth)
            {
                hmkp_l->Fill(kplus_lo[j]->PdgCode()); // fills Histogram with only one value
        ...
        {
            TVector3 ang1 = D0_lo[j]->GetPosition();
            hD0m_lpid->Fill(D0_lo[j]->M());
            hD0_momentum->Fill(D0_lo[j]->P());
            hD0_costheta->Fill(ang1.CosTheta());
            hD0_phi->Fill(ang1.Phi());
        ...
        {
            TVector3 ang2 = D0bar_lo[j]->GetPosition();
            hD0barm_lpid->Fill(D0bar_lo[j]->M());
            hD0bar_momentum->Fill(D0bar_lo[j]->P());
            hD0bar_costheta->Fill(ang2.CosTheta());
            hD0bar_phi->Fill(ang2.Phi());
            D0bar_lo.Select(D0MassSel);
        }

        RhoCandidate *D0_tmp;
        RhoCandidate *D0bar_tmp;
        RhoCandidate *kp;
        RhoCandidate *pim;
        RhoCandidate *pip;
        RhoCandidate *km;
        ...
        for (j = 0; j < pbarpSystem_lo.GetLength(); ++j)
        {
            ...
            km = D0bar_tmp->Daughter(0);
            pip = D0bar_tmp->Daughter(1);
        }
        ...
        {
            if (
                (kaon_plus_id == kplusMcTruth->PdgCode()) &&
                (kaon_minus_id == kminusMCTruth->PdgCode()) &&
                (pion_plus_id == piplusMcTruth->PdgCode()) &&
                (pion_minus_id == piminusMcTruth->PdgCode()))
            {
                // Fill RhoCandidates with Mother-Particles

                RhoCandidate *kp_mother_MC = kp->GetMcTruth()->TheMother();
                ...
                if (
                    (kp_mother_MC->PdgCode() == D0_id) &&
                    (pim_mother_MC->PdgCode() == D0_id))
                {
                ...
                if (
                    (km_mother_MC->PdgCode() == D0bar_id) &&
                    (pip_mother_MC->PdgCode() == D0bar_id))
                {
