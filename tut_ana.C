...
TFile *out = TFile::Open("D0_kpi_ana.root", "RECREATE");
...
void tut_ana(int nevts = 0, TString prefix = "D0_kpi")
{
    ...
    while (theAnalysis->GetEvent() && i++ < nevts)
    {
        ...
        for (j = 0; j < kplus.GetLength(); ++j)
        {
            ...
            RhoCandidate *kplusMother = kplus[j]->GetMcTruth()->TheMother();
            // if(kplusMcTruth){
            // RhoCandidate *kplusMother = kplusMcTruth->TheMother();
            if (kplusMother)
            {
                hmkp->Fill(kplusMother->PdgCode()); // Fills a Histogram with all K+ Mothers from MC
            }
        }
    }
    // kminus loop
